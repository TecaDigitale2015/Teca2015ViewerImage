...
/**
 * Questa classe viene utilizzata per gestire l'interfaccia verso di Database
 * Teca per la visualizzazione delle immagini
 * 
...
public class ImageViewerTecaDigitale extends ImageViewerTecaDigitaleIIPImage {
	...
	/**
	 * Indica se l'immagine contiene Carte geografiche gestire dall'interfaccia
	 * xlimage
	 */
	...
	private static QueryResponse findSolr(String query, int start, int rows)
			throws NumberFormatException, ConfigurationException, SolrException, SolrServerException {
		...
		try {
			find = new FindDocument(Configuration.getValue("solr.URL"),
					...
					Configuration.getValue("solr.optional"));
			if (query == null) {
				query = "*:*";
			}
			response = find.find(query, start, rows);
	...
	/**
	 * @see mx.imageviewer.servlet.interfacie.IImageViewer#initPage(HttpServletRequest
	 *      request, HttpServletResponse response)
	 */
	@Override
	public ImageViewer initPage(HttpServletRequest request, HttpServletResponse response, String serverName)
		...
		try {
			...
			if (request.getParameter("id") != null || request.getParameter("idr") != null) {
				...
				imageViewer.setTitolo("Visualizzatore Immagini TecaDigitale ver. 5.0.1");
...
//				}
	 ...
	 */
	public static SolrDocumentList findSolr(String id) throws NumberFormatException, ConfigurationException,
			SolrException, SolrServerException, ImageViewerException {
		...
		try {
			if (solrResponse.getNumFound() > 0) {
				...
				bais = new ByteArrayInputStream(
						((ArrayList<String>) solrResponse.get(0).get(ItemTeca.XML)).get(0).getBytes());
	...
	private Xlimage checkXlimage(String idr) throws ServletException {
		...
		// pagina = new Pagina();
		//
		// pagina.setUrlPage((String)Configuration.get("bncf.xlimage.urlPage",
		// "http://opac.bncf.firenze.sbn.it/php/xlimage/XLImageRV.php?&amp;xsize=500&amp;image=")+
		// rs.getString("imgPathName"));
		// pagina.setLink((String)
		// Configuration.get("bncf.xlimage.link","javascript:changeImg")+"('"+xlimage.getPagina().size()+"')");
	...
	private String getBookIdr(SolrDocumentList solrResponse) throws ImageViewerException {
		...
		// TODO: da implementare tutte le casistiche in cui si visualizzano i dati nelle
		// pagine interne
		...
		//
		// if (!lavorato){
		// throw new ImageViewerException("Non risultano presenti collegamenti con le
		// immagini");
		// }
		// } catch (SQLException e) {
		// log.error(e);
	...
	private boolean isCostola(Metadigit mag) {
		...
		// view.addWhere(" AND (LOWER(TBLRIS.RISNOTAPUB) like '%costola%' OR
		// LOWER(TBLRIS.RISNOTAPUB) like '%dorso%')");
		// rs = view.startSelect();
		// result = rs.next();
		// }
		// catch (SQLException e)
	...
	/**
	 * Metodo utilizzato per ricavare se l'opera in oggetto è corrispondente ad una
	 * collezione
	 * 
	 * @param ris
	 * @return
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void isCollezione(SolrDocumentList solrResponse, ImageViewer imageViewer, String serverName)
			throws ImageViewerException {
		...
		try {
			...
			if (((ArrayList<String>) solrResponse.get(0).get(ItemTeca.TIPOOGGETTO + "_show")).get(0)
					.equals(ItemTeca.TIPOOGGETTO_COLLANA)) {
				if (((String) Configuration.getValueDefault("imageViewer." + serverName + ".externalVolumi",
						Configuration.getValueDefault("imageViewer.ALL.externalVolumi", "false")))
								.equalsIgnoreCase("flase")) {
			...
			// if (((String) Configuration.get("imageViewer." + serverName +
			// ".externalVolumi",
			// Configuration.get("imageViewer.ALL.externalVolumi",
			// "false"))).equalsIgnoreCase("flase")){
			// tblRelRis = new TblRelRis(cp);
			...
			// }
			// }else{
			// throw new ImageViewerException("Non risulta presente l'oggetto in base
			// dati");
			// }
		} catch (ConfigurationException e) {
			throw new ImageViewerException(e.getMessage(), e);
	...
	/**
	 * 
	 * @see mx.imageviewer.servlet.interfacie.IImageViewer#showImage(javax.servlet.http.HttpServletRequest,
	...
	// public void showImage(HttpServletRequest request, HttpServletResponse
	...
	//
	// fileImg= "/Volumes/HDDSORGENTI/Backup/discoc/Progetti/Tivoli/Tivoli
	// DVD/Immagini/P000001_SIGI_1691_D1_VIII_12/200";
	// fileImg += File.separator;
	// fileImg += "TIV_"+ConvertText.mettiZeri(request.getParameter("sequence"),
	// 4)+".jpg";
	//
	...
	/**
	 * 
	 * @see mx.imageviewer.servlet.interfacie.IImageViewer#readBook(javax.servlet.http.HttpServletRequest,
	 *      javax.servlet.http.HttpServletResponse)
	 */
	@SuppressWarnings("unchecked")
	@Override
	public ReadBook readBook(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		try {
			...
			if (request.getParameter("usage") != null)
				...
				usage = (String) Configuration.getValueDefault(
						"imageViewer." + request.getServerName() + ".usageImageDefault",
						Configuration.getValueDefault("imageViewer.ALL.usageImageDefault", "3"));

			if (request.getParameter("numPage") != null) {
				numPage = Integer.valueOf(request.getParameter("numPage"));
			}
			if (request.getParameter("idr") != null) {
				solrResponse = findSolr(request.getParameter("idr"));
				mag = readXml(solrResponse);
				...
				myReadBook = readBook.esegui(request.getParameter("idr"), mag, usage, numPage,
						new File(((ArrayList<String>) solrResponse.get(0).get(ItemTeca.ORIGINALFILENAME+"_show")).get(0)));
			}
		} catch (NumberFormatException e) {
			log.error(e.getMessage(), e);
	...
	@Override
	public Opera readCatalogo(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		return readCatalogo(request, response, true);
	}

	public Opera readCatalogo(HttpServletRequest request, HttpServletResponse response, boolean initBook)
			throws ServletException, IOException {
		Opera opera = null;
		// ConnectionPool cp = null;
		// MsSqlPool msp = null;
		...
		// !libro.getAutore().equals(rs.getString("tmpautore")))){
		//
		// if (gruppo != null) {
	...
	@Override
	public ImageViewer showCatalogo(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		ImageViewer imageViewer = null;
		// Opera opera = null;
		// String idr = "";
	 ...
	 */
	@Override
	public Opera readStru(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		Opera opera = null;
		Metadigit mag = null;
		boolean costola = false;
	...
	private void checkStru(Libro libro, Stru stru, boolean costola) throws ServletException {
		...
		if (stru.getStru() != null && stru.getStru().size() > 0) {
			...
			if (stru.getStart() != null) {
				for (int x = stru.getStart().intValue(); x <= (stru.getStop() == null ? stru.getStart().intValue()
						: stru.getStop().intValue()); x++) {
					volume = new Volume();
				...
				if (stru.getElement() != null && stru.getElement().size() > 0) {
	...
	// private void checkStru(Libro libro, String idr, String idrPadre, boolean
	// costola) throws ServletException{
	// Vector<String[]> stru = null;
	// Libro subLibro = null;
	// Volume volume = null;
	// int idPagina = 0;
	...
	// private Vector<String[]> readStru(String idr, String tipoRelId) throws
	// ServletException{
