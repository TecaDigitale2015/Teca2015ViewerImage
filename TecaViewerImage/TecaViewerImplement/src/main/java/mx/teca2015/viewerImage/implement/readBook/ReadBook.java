...
public class ReadBook {
	...
	// public mx.imageviewer.schema.gestionelibro.ReadBook esegui(String risIdr,
	// String usage)
			...
			if (mag.getImg() != null && mag.getImg().size() > 0) {
				...
				if (numPage != null) {
					...
					if (checkUsage(mag.getImg().get(numPage).getUsage(), usage)) {
						...
						sImg = mag.getImg().get(numPage).getFile().getHref();
						...
						for (int y = 0; y < mag.getImg().get(numPage).getAltimg().size(); y++) {
							if (checkUsage(mag.getImg().get(numPage).getAltimg().get(y).getUsage(), usage)) {
								nomenclatura = mag.getImg().get(numPage).getNomenclature();
								sImg = mag.getImg().get(numPage).getAltimg().get(y).getFile().getHref();

								if (mag.getImg().get(numPage).getAltimg().get(y).getImageDimensions() != null) {
									if (mag.getImg().get(numPage).getAltimg().get(y).getImageDimensions()
											.getImagelength() != null
											&& mag.getImg().get(numPage).getAltimg().get(y).getImageDimensions()
													.getImagewidth() != null) {
										imgLength = mag.getImg().get(numPage).getAltimg().get(y).getImageDimensions()
												.getImagelength().intValue();
										imgWidth = mag.getImg().get(numPage).getAltimg().get(y).getImageDimensions()
												.getImagewidth().intValue();
									} else {
										imgLength = 800;
										imgWidth = 600;
									}
					...
					if (nomenclatura != null) {
						if (sImg.startsWith("./")) {
							sImg = sImg.substring(2);
						} else if (sImg.startsWith(".")) {
							sImg = sImg.substring(2);
						}
						fImg = new File(fMag.getParentFile().getAbsolutePath() + File.separator + sImg);
						immagini.getImmagine().add(addImmagine(id, numPage, isCostola, nomenclatura, imgLength,
								imgWidth, usage, fImg.getAbsolutePath()));
					}
				} else {
					for (int x = 0; x < mag.getImg().size(); x++) {
						...
						if (checkUsage(mag.getImg().get(x).getUsage(), usage)) {
							...
							sImg = mag.getImg().get(x).getFile().getHref();
							...
							for (int y = 0; y < mag.getImg().get(x).getAltimg().size(); y++) {
								if (checkUsage(mag.getImg().get(x).getAltimg().get(y).getUsage(), usage)) {
									nomenclatura = mag.getImg().get(x).getNomenclature();
									sImg = mag.getImg().get(x).getAltimg().get(y).getFile().getHref();
									if (mag.getImg().get(x).getAltimg().get(y).getImageDimensions() != null) {
										if (mag.getImg().get(x).getAltimg().get(y).getImageDimensions()
												.getImagelength() != null
												&& mag.getImg().get(x).getAltimg().get(y).getImageDimensions()
														.getImagewidth() != null) {
											imgLength = mag.getImg().get(x).getAltimg().get(y).getImageDimensions()
													.getImagelength().intValue();
											imgWidth = mag.getImg().get(x).getAltimg().get(y).getImageDimensions()
													.getImagewidth().intValue();
										} else {
											imgLength = 800;
											imgWidth = 600;
						...
						if (nomenclatura != null) {
							...
							fImg = new File(fMag.getParentFile().getAbsolutePath() + File.separator + sImg);
							immagini.getImmagine().add(addImmagine(id, x, isCostola, nomenclatura, imgLength, imgWidth,
									usage, fImg.getAbsolutePath()));
						}
					}
				}
				if (immagini != null) {
	...
	public static Format getFormat(Img img, Gen gen) {
		Format result = null;

		if (img.getFormat() != null) {
			result = img.getFormat();
		} else {
			if (img.getImggroupID() != null) {
				if (gen.getImgGroup() != null) {
					for (ImgGroup imgGroup : gen.getImgGroup()) {
						if (((String) imgGroup.getID()).equals(((ImgGroup) img.getImggroupID()).getID())) {
							result = imgGroup.getFormat();
							break;
						}
					}
				}
	...
	public static Format getFormat(Altimg altimg, Gen gen) {
		Format result = null;

		if (altimg.getFormat() != null) {
			result = altimg.getFormat();
		} else {
			if (altimg.getImggroupID() != null) {
				if (gen.getImgGroup() != null) {
					for (ImgGroup imgGroup : gen.getImgGroup()) {
						if (((String) imgGroup.getID()).equals(((ImgGroup) altimg.getImggroupID()).getID())) {
							result = imgGroup.getFormat();
							break;
						}
					}
	...
	// private void updImgConv(int imgLengthConv, int imgWidthConv, String idTblImg)
	...
	private Immagine addImmagine(String id, int sequenza, Boolean visPaginaDoppia, String nomenclatura, int altezza,
			int larghezza, String usage, String fileImg) {
		...
		if (fileImg != null) {
			try {
				if (Configuration.getValue("prefixStorage") != null
						&& !Configuration.getValue("prefixStorage").trim().equals("")) {
					fileImg = fileImg.replace(Configuration.getValue("prefixStorage"), "");
				}
			} catch (ConfigurationException e) {
			}
			if (fileImg.startsWith("/")) {
				fileImg = fileImg.substring(1);
			}
			fileImg = fileImg.replace("/", "!");
			immagine.setFileIIIF(fileImg);
		}
		return immagine;
	}
	 ...
	 * @throws SQLException
	 */
	// private DatiBibliografici initDatiBibliografici(String risidr) throws
	// SQLException
	private DatiBibliografici initDatiBibliografici(Metadigit mag) throws ConfigurationException {
		...
		try {
			...
			if (mag.getBib() != null) {
				...
				if (!dati.equals("")) {
					datiBibliografici.setTitolo(dati);
				}
				...
				if (!dati.equals("")) {
					datiBibliografici.setPubblicazione(dati);
				}
				...
				if (mag.getBib().getPiece() != null) {
					if (mag.getBib().getPiece().getYear() != null && mag.getBib().getPiece().getIssue() != null) {
				...
				// datiBibliografici.setUnitaFisica(rs.getString("pieceGr").trim()+"
				// "+rs.getString("PieceDt").trim());
				//
				urlDatiBibliografici = (String) Configuration.getValueDefault(
	...
	private String read(List<SimpleLiteral> value) {
		...
		if (value != null && value.size() > 0) {
			for (int x = 0; x < value.size(); x++) {
				if (value.get(x).getContent() != null && value.get(x).getContent().size() > 0) {
